...
export function buildHeavyTaskSystemPromptPolicy(
  ...
  return [
    ...
    '- Before final self_check_submit, run a public workspace hygiene guard over relevant deliverable directories: compare before/after listing or diff output, summarize checked paths and added/modified/removed files in self_check_submit.executionHygiene.workspaceGuard, and do not treat a self-check with uncleaned added files as a strong pass.',
    '- Heavy-task finalization may be rejected once before official verification when the latest self_check_submit is missing, failing, inconclusive, lacks sandbox/workspace guard evidence, or does not address visible required artifacts. Use the bounded repair/check prompt to run public checks, repair if needed, and resubmit self_check_submit.',
