...
import type { HeavyTaskSelfCheckPlanState, HeavyTaskSemanticSelfCheckState, HeavyTaskSourceGuardResult } from '../task-contracts.js';
...
describe('heavy-task semantic self-check tools', () => {
  test('self_check_plan_submit records accepted public plan state', async () => {
    ...
    assert.equal(result.accepted ? result.plan.finalArtifacts[0]?.path : undefined, '/app/move.txt');
    ...
    assert.equal(projection.latestHeavyTaskSelfCheckPlan?.planId, result.accepted ? result.plan.planId : undefined);
    ...
    const result = await selfCheckSubmit.impl({
      ...
      executionHygiene: {
        sandbox: {
          ...
          publicReason: 'public inputs were copied into the sandbox and generated outputs stayed there',
        },
        scratchUsed: true,
  ...
  test('rejects private or evaluator-only material before appending accepted state', async () => {
    const privateInputs = [
      ...
      { publicReason: 'public check', commandEvidence: [{ command: 'npm test' }], executionHygiene: { workspaceGuard: { checked: true, addedPaths: ['hidden/tests/leak.log'] } } },
      { publicReason: 'public check', commandEvidence: [{ command: 'npm test' }], executionHygiene: { workspaceGuard: { beforeListingCommand: 'find official-verifier-output.json' } } },
      { publicReason: 'public check', commandEvidence: [{ command: 'npm test' }], executionHygiene: { sandbox: { root: 'hidden/tests/sandbox' } } },
      { publicReason: 'public check', commandEvidence: [{ command: 'npm test' }], executionHygiene: { sandbox: { root: '/tmp/maka-self-check', inputPaths: ['official-verifier-output.json'] } } },
    ];
  ...
  test('self_check_plan_submit source guard rejects private or evaluator-only material in all plan fields', () => {
    const privatePlans = [
      ...
      { finalArtifacts: [{ path: '/app/move.txt', purpose: 'private scoring criteria', publicReason: 'public task requires it' }] },
      { selfCheckScratch: { root: '/tmp/maka-self-check', expectedGeneratedPaths: ['official-verifier-output.json'], publicReason: 'public scratch' } },
      { workspaceGuardPlan: { checkedPaths: ['/app'], expectedAddedPaths: ['hidden/tests/leak.txt'], publicReason: 'public guard' } },
      { workspaceGuardPlan: { checkedPaths: ['/app'], expectedGeneratedPathsOutsideScratch: ['/app/out'], publicReason: 'evaluator-only fixture mentioned it' } },
    ];
    ...
    for (const override of privatePlans) {
      ...
      assert.equal(validation.guard.status, 'rejected');
      assert.doesNotMatch(validation.guard.publicReason, /official-verifier-output|private scoring criteria|evaluator-only fixture|leak\.txt/);
    }
  });
  ...
  test('audits planned additions separately from unplanned scratch escapes', () => {
    ...
    const planned = acceptedSelfCheck('self-check-planned', 'pass', 'test -f /app/move.txt passed on public files.');
    ...
    planned.commandEvidence = [{ command: 'test -f /app/move.txt', exitCode: 0, artifactRefs: ['/app/move.txt'] }];
    ...
    const readOnlyInput = acceptedSelfCheck('self-check-readonly-input', 'pass', 'checked public input file.');
    readOnlyInput.commandEvidence = [{ command: 'python /app/input.py --check /app/move.txt', exitCode: 0 }];
    ...
    const unplanned = acceptedSelfCheck('self-check-unplanned', 'pass', 'compiled public source to /app/polyglot/cmain.');
    unplanned.commandEvidence = [{ command: 'cc /app/polyglot/main.py.c -o /app/polyglot/cmain', exitCode: 0 }];
    unplanned.artifactEvidence = [{ path: '/app/polyglot/cmain', kind: 'generated_output', exists: true }];
...
function planInput(overrides: Partial<HeavyTaskSelfCheckPlanSubmitInput> = {}): HeavyTaskSelfCheckPlanSubmitInput {
